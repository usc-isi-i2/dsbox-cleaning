...
class DateExtractor():
    ...
    def extract(self, text: str = None,
        ...
        Args:
            ...
            relative_base: datetime.datetime - offset relative dates detected based on
            'relative_base'
            ...
            prefer_day_of_month: enum['first', 'current', 'last'] - use which day of the month
            when there is no 'day'
            prefer_dates_from: enum['past', 'current', 'future'] - use which date when there is
            few info(e.g. only month)
            date_value_resolution: enum[DateResolution.SECOND, DateResolution.MINUTE,
            DateResolution.HOUR,
            ...
            if cur_max['end'] <= x['start']:
                ...
                if len(x['value']) > len(cur_max['value']):
                    ...
                    if x['order'] in ['SINGLE_YEAR']:
                        ...
                        if self.settings[PREFER_LANGUAGE_DATE_ORDER] and \
                                self.lan in language_date_order:
    ...
    def parse_date(self, date_info: dict) -> datetime.datetime or None:
        ...
        if date_info['pattern']:
            ...
            if re.match(possible_illegal, date_info['value']):
                return None
            elif re.match(r'^[0-9]{4}$', date_info['value']) and len(
                    [g for g in date_info['groups'] if g]) > 1:
                return None
        ...
        for s in date_info['groups']:
            if s:
                p = self.symbol_list[date_info['order']][i] if not user_defined_pattern else \
                    user_defined_pattern[i]
                if p in units['D']:
                    miss_day = False
                if p in units['M']:
                    miss_month = False
                ...
                formatted.append(
                    re.sub(r'[^0-9+\-]', '', formatted_str) if p == '%z' else formatted_str)
            i += 1
        ...
        if formatted and pattern:
            try:
                ...
                try:
                    date = datetime.datetime.strptime('-'.join(formatted[:-1]),
                                                      '-'.join(pattern[:-1]))
                except ValueError:
            ...
            if miss_year and miss_month and miss_day:
                ...
                if miss_week:
                    ...
                    target = day_of_week_to_number[
                        week_of_day] if week_of_day in day_of_week_to_number \
                        else today.weekday()
    ...
    def post_process_date(self, date: datetime.datetime) -> datetime.datetime or None:
        ...
        if not date.tzinfo and self.settings[RETURN_AS_TIMEZONE_AWARE]:
            ...
            try:
                ...
                warn('DateExtractor: Failed to set timezone as ' + str(
                    self.default_tz) + '. Catch ' + str(e))
        elif not self.settings[RETURN_AS_TIMEZONE_AWARE]:
            date = date.replace(tzinfo=None)

        try:
            if (self.settings[IGNORE_DATES_BEFORE] and date < self.settings[IGNORE_DATES_BEFORE]) \
                    or \
                    (self.settings[IGNORE_DATES_AFTER] and date > self.settings[
                        IGNORE_DATES_AFTER]):
                return None
        ...
        if self.settings[TO_TIMEZONE] and self.settings[RETURN_AS_TIMEZONE_AWARE]:
            try:
                ...
                warn('DateExtractor: Failed to set timezone as ' + str(
                    self.settings[TIMEZONE]) + '. Catch ' + str(e))
    ...
    def convert_to_iso_format(date: datetime.datetime,
                              resolution: DateResolution = DateResolution.DAY) -> str or None:
        """
        ...
        """
        # TODO: currently the resolution is specified by the user, should it be decided what we
        # have extracted,
